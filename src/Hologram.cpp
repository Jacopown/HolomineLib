...
Hologram::Hologram(const SignalLog &signalLog, int xStep, int yStep) {
  ...
  width = static_cast<int>(
              std::ceil((signalLog.getWidth() - signalLog.getMinX()) / xStep)) +
          1;
  height = static_cast<int>(std::ceil(
               (signalLog.getHeight() - signalLog.getMinY()) / yStep)) +
           1;
  std::cout << "Width " << signalLog.getWidth() << "\n";
  std::cout << "Height " << signalLog.getHeight() << "\n";
...
void Hologram::saveImage(const std::string &path, bool real) {
  ...
  cv::Mat visual;
  real ? cv::normalize(channels[0], visual, 0, 255, cv::NORM_MINMAX, CV_8UC1)
       : cv::normalize(channels[1], visual, 0, 255, cv::NORM_MINMAX, CV_8UC1);
